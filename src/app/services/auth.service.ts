...
export class AuthService {
	...
	setAuthStorage(auth: any) {
		if ( typeof auth.data !== 'undefined' ) {
			localStorage.setItem('intranet_user', btoa(JSON.stringify(auth.data)));
		} else {
			localStorage.setItem('intranet_user', btoa(JSON.stringify(auth)));
		}
		this.auth = auth;
	}
	...
	getAuthStorage() {
		...
		if ( localStorage.getItem('otp_account_type') ) {
			user = JSON.parse(atob(localStorage.getItem('intranet_user')));
		} else {
			user = JSON.parse(atob(localStorage.getItem('intranet_user')));
		}
